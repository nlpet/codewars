...
words; for our purposes "AAIILNORSTTY" is also a "word" composed of the same
...
def listPosition(word):
    ...
    for i in range(lw):
        ...
        for j in counter:
            if j < w:
                pos += (suffix * counter[j]) // counter[w]
        suffix = (suffix * (i + 1)) // counter[w]
...
def run_tests():
    with Test() as test:
        ...
        testValues = {'A': 1, 'ABAB': 2, 'AAAB': 1,
                      'BAAA': 4, 'QUESTION': 24572, 'BOOKKEEPER': 10743}
        for word in testValues:
            test.assert_equals(listPosition(
                word), testValues[word], 'Incorrect list position for: ' + word)
