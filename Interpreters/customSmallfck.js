/*
  Problem location: cw/esolang-interpreters-number-2-custom-smallfuck-interpreter
  ...
  fellow Codewarrior, these three high-level inter-related Kata gradually evolved
  ...
  is a sized-down variant of the famous Brainfuck Esolang. Key differences include:
  ...
  * It does not define input or output - the "input" is encoded in the initial state
    of the data storage (tape) and the "output" should be decoded in the final state
    of the data storage (tape)
  ...
  * The pointer goes out-of-bounds (i.e. if it moves to the left of the first
  ...
  Implement a custom Smallfuck interpreter interpreter() which accepts the
  following arguments:
  ...
  For example, if the string "00101100" is passed in then it should translate to
  something of this form within your interpreter: [0, 0, 1, 0, 1, 1, 0, 0]. You may
  assume that all input strings for tape will be non-empty and will only contain "0"s
  and "1"s.
  ...
  in your interpreter ends up being [1, 1, 1, 1, 1] then return the string "11111".
*/
...
function interpreter(code, tape) {
  ...
  const res = tape.split('');
  const [sf, sb] = getLoops(code);

  while ((ptr >= 0 && ptr < tape.length) && (i < code.length)) {
    let cmd = code[i];
...
function test() {
  let msg = 'Failed to flip the leftmost cell of the tape';
  assert(interpreter("*", "00101100") === "10101100", msg);

  msg = 'Failed to flip the second and third cell of the tape';
  assert(interpreter(">*>*", "00101100") === "01001100", msg);

  msg = 'Failed to flip all the bits in the tape';
  assert(interpreter("*>*>*>*>*>*>*>*", "00101100") === "11010011", msg);

  msg = 'Failed to flip all the bits that are initialised to 0';
  assert(interpreter("*>*>>*>>>*>*", "00101100") === "11111111", msg);

  msg = 'Failed to flip all bits that are initialised to 1, right -> left';
